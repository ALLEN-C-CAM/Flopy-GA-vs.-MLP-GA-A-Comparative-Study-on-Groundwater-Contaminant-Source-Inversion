...
D3 = 4  # 参数个数（前8个应力期S1的释放浓度）
D4 = 4  # 参数个数（前8个应力期S2的释放浓度）
N = 200  # 拉丁超立方层数
bounds1 = [[2, 5], [2, 5]]  # 参数对应范围（S1释放行位置范围为3到5，S1释放列位置范围为3到5）
bounds2 = [[5, 8], [2, 5]]  # 参数对应范围（S2释放行位置范围为6到8，S2释放列位置范围为3到5）
bounds3 = [[0, 70]] * 4  # 参数对应范围（S1释放浓度范围为0到70）
bounds4 = [[0, 70]] * 4  # 参数对应范围（S2释放浓度范围为0到70）
