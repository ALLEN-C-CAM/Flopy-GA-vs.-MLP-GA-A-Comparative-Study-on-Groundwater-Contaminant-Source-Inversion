...
np.random.seed(42)
class NN_MonitorModel(nn.Module):
    def __init__(self, input_dim=12, output_dim=60, hidden_dim=256):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, hidden_dim),
            nn.BatchNorm1d(hidden_dim),
...
def train_model():
    ...
    # 加载验证集
    val_X_np = pd.read_excel('data/模型输入数据_验证1.xlsx', header=None).values
    val_Y_np = pd.read_excel('data/模型输出数据_验证1.xlsx', header=None).values
    ...
    # 创建模型
    model = NN_MonitorModel(input_dim=12, output_dim=60, hidden_dim=256)
    ...
    # 损失曲线
    plt.figure(figsize=(10, 5))
    plt.semilogy(train_losses, label='数据损失')
    plt.semilogy(val_losses, label='MRE验证损失')
    plt.xlabel('Epoch')
    plt.ylabel('损失 (log scale)')
    plt.legend()
    plt.title('训练与验证损失曲线')
