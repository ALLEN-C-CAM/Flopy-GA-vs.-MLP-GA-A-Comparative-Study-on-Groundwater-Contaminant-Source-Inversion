...
import sys
import os
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '数据驱动')))
...
class nn:
    @staticmethod
    def predict_monitor_points(model, new_input, scaler_X_mean, scaler_X_scale, scaler_Y_mean=None,
                               scaler_Y_scale=None):
    ...
    def load_saved_model(path=None):
        if path is None:
            path = os.path.abspath(os.path.join(os.path.dirname(__file__), '..', '数据驱动', 'nn_model.pth'))
        checkpoint = torch.load(path, weights_only=True)
        model = NN_MonitorModel()
        model.load_state_dict(checkpoint['model_state_dict'])
        return model, checkpoint['scaler_X_mean'], checkpoint['scaler_X_scale'], checkpoint.get(
            'scaler_Y_mean'), checkpoint.get('scaler_Y_scale')
    ...
    def evaluate_model(y_true, y_pred):
        ...
        r2 = (np.sum(TM * TP) ** 2) / (np.sum(TM ** 2) * np.sum(TP ** 2)) if np.sum(TM ** 2) * np.sum(
            TP ** 2) != 0 else 0
        nse = 1 - np.sum((y_true - y_pred) ** 2) / np.sum((y_true - np.mean(y_true)) ** 2) if np.sum(
            (y_true - np.mean(y_true)) ** 2) != 0 else 0
...
class Objective_function(ElementwiseProblem):
    def __init__(self):
        super().__init__(
            n_var=12, n_obj=1, n_constr=0,
            xl=np.array([2, 4, 4, 4] + [10.00] * 16),
            xu=np.array([4, 4, 4, 4] + [100.00] * 16)
        )
...
class OptimizationCallback(Callback):
    def __init__(self, interval=10):
        ...
        self.mre_history = []  # 记录每次迭代的MRE
        self.generation_history = []  # 记录迭代次数
    ...
    def notify(self, algorithm):
        ...
        # 获取当前迭代的最优MRE
        best_f = np.min(algorithm.pop.get("F"))
        self.mre_history.append(best_f)
...
class Optimization:
    ...
    def run(self):
        ...
        algorithm = GA(
            ...
            mutation=PM(prob=0.05, eta=3, vtype=float),
            repair=RoundingRepair(),
            eliminate_duplicates=True
        )
        res = minimize(problem, algorithm, termination=('n_gen', 500), seed=1, callback=callback, verbose=False)
        best_solution = res.X
        prediction = nn.predict_monitor_points(self.model, best_solution, self.mean, self.scale, self.y_mean,
                                               self.y_scale)
        ...
        self.save_mre_history_to_excel(callback.generation_history, callback.mre_history)  # 保存MRE历史到Excel
    ...
    def save_prediction_to_excel(self, y_true, y_pred, residuals, mre_values, filename='data/预测结果2.xlsx'):
        df = pd.DataFrame({
            ...
            '预测值': y_pred,
            '残差': residuals,
            '相对误差 (%)': np.abs((residuals / y_true)) * 100 if np.mean(np.abs(y_true)) != 0 else np.zeros_like(
                y_true)
        })
    ...
    def save_mre_history_to_excel(self, generations, mre_values, filename='data/mre_history.xlsx'):
        """将每次迭代的MRE保存到Excel"""
        df = pd.DataFrame({
            '迭代次数': generations,
            '最优MRE (%)': mre_values
        })
        df.to_excel(filename, sheet_name='MRE历史', index=False)
        print(f"✅ 迭代MRE历史已保存到 {filename}")
