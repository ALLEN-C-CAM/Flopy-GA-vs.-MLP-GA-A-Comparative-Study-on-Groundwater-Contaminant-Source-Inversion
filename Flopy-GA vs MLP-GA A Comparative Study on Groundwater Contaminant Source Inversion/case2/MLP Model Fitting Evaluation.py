...
class ModelEvaluator:
    ...
    def evaluate_model(self, y_true, y_pred):
        ...
        # 返回 RMSE、R² 和 NSE
        return rmse, r2, nse, residuals


def evaluate_model():
    # 加载验证数据
    try:
        data_X_val_np = pd.read_excel('./data/模型输入数据_验证.xlsx', header=None).values
        data_Y_val_np = pd.read_excel('./data/模型输出数据_验证.xlsx', header=None).values
    ...
    # 使用自定义评估方法计算指标
    rmse, r2, nse, residuals = evaluator.evaluate_model(data_Y_val_np, predictions)
    ...
    # 使用Unicode字符²代替LaTeX渲染
    plt.text(0.05, 0.95, f'RMSE = {rmse:.4f}', transform=plt.gca().transAxes)
    plt.text(0.05, 0.90, f'R² = {r2:.4f}', transform=plt.gca().transAxes)  # 直接使用²字符
    plt.text(0.05, 0.85, f'NSE = {nse:.4f}', transform=plt.gca().transAxes)
    ...
    print(f"✅ 评估完成，散点图保存至: scatter_plot.png")
